 ...
 * PUT    → update record          { id, ...fields }   | replace singleton { ...fields }
 * PATCH  → bulk reorder           { order: [id, id, ...] }
 * DELETE → remove record          ?id=
 */
const COLLECTIONS = ["navigation", "categories", "automations", "packages", "socialLinks", "faqs", "requests", "messages", "posts", "inbox", "clients", "accounts"] as const;
const SINGLETONS = ["settings", "socialPage"] as const;
type Col = (typeof COLLECTIONS)[number];
type Single = (typeof SINGLETONS)[number];

const isCol = (s: string): s is Col => (COLLECTIONS as readonly string[]).includes(s);
const isSingle = (s: string): s is Single => (SINGLETONS as readonly string[]).includes(s);

function revalidateAll() {
  ["/", "/automations", "/social-media", "/social", "/about", "/contact", "/request", "/request/social-media"].forEach((p) =>
    revalidatePath(p)
  );
...
export async function GET(_: Request, { params }: { params: { collection: string } }) {
  const g = guard(); if (g) return g;
  const db = getDb();
  const c = params.collection;
  if (isCol(c)) return NextResponse.json(db[c]);
  if (isSingle(c)) return NextResponse.json(db[c]);
...
export async function PUT(req: Request, { params }: { params: { collection: string } }) {
  ...
  const body = await req.json();
  if (isSingle(c)) {
    mutateDb((db) => { (db as Db)[c] = { ...(db[c] as object), ...body } as never; });
    revalidateAll();
